...
\begin{figure}[htbp!]
\centering
%TODO: RESIZE THIS IF THERE ARE MORE SPACE THAN NEEDED!
...
Bei funkbasierten \gls{etcs}-Verbindungen übernimmt \textit{EuroRadio} die sicherheitsbezogene Absicherung der zwischen Fahrzeug und \gls{rbc} übertragenen \gls{etcs}-Nachrichten. Während \gls{gsm-r} die Funk- und Übertragungsinfrastruktur bereitstellt, schützt \textit{EuroRadio} insbesondere die Authentizität und Integrität der sicherheitskritischen Anwendungsdaten~\parencite[S. 744, 751]{chothiaAttack2017}.

Im traditionellen \textit{EuroRadio}-Protokoll erfolgt dieser Schutz über den \textit{EuroRadio}-\gls{mac}-Algorithmus, eine Modifikation des \gls{iso} 9797-1 \gls{mac}-Algorithmus 3. Dieser berechnet einen 64-Bit-\gls{mac} mittels \gls{cbc}-Verfahren unter Nutzung von \textit{Single}-\gls{des} für die ersten $n-1$ Blöcke und \textit{Triple}-\gls{des} (\gls{3des}) für den finalen Block~\parencite[S. 751]{chothiaAttack2017}. Die Verwaltung der symmetrischen Sitzungsschlüssel (\gls{kmac}) erfolgt über ein hierarchisches \textit{Key Management System} (\gls{kms}) mit streckenseitigen \textit{Key Management Centren} (\gls{kmc})~\parencite[S. 90, 100]{trinckaufETCS2024}.

In neueren \gls{etcs}-Spezifikationen wird die Absicherung der Zug-Strecken-Kommunikation zunehmend auf moderne \gls{ip}-basierte Sicherheitsmechanismen ausgerichtet. Dabei kommen insbesondere \gls{tls}-basierte Verfahren und eine zertifikatsbasierte \textit{Public Key Infrastructure} (\gls{pki}) zum Einsatz~\parencite[S. 105, 211]{trinckaufETCS2024}.
...
\subsection{Stand der Forschung und verwandte Arbeiten} \label{sec:Stand_der_Forschung_und_verwandte_Arbeiten}
Die vorliegende Hausarbeit baut auf einer Reihe zentraler wissenschaftlicher Veröffentlichungen, technischer Standards und Risikoanalysen auf, die Teilaspekte der sicherheitskritischen Eisenbahnkommunikation untersuchen. Die relevanten Arbeiten lassen sich in vier thematische Schwerpunkte gliedern:

\subsubsection{Kryptographische Protokollanalysen und Kommunikationsschwachstellen}
\begin{itemize}
    \item \textbf{Chothia et al. (2017) -- \textit{An Attack Against Message Authentication in the ERTMS Train to Trackside Communication Protocols}:}
    Chothia et al. untersuchen die kryptographische Absicherung der Zug-Strecken-Kommunikation in \gls{ertms} und zeigen, dass der traditionelle \textit{EuroRadio}-Integritätsschutz trotz vorhandener Authentifizierungsmechanismen kryptographische Schwächen aufweist~\parencite[S. 743--756]{chothiaAttack2017}. Insbesondere ermöglicht die Kombination aus 64-Bit-Blocklänge und \gls{des}-basierter Konstruktion kollisionsbasierte Angriffe, durch die unter bestimmten Voraussetzungen gefälschte sicherheitskritische Nachrichten erzeugt werden können~\parencite[S. 751--754]{chothiaAttack2017}. Für die vorliegende Arbeit ist dieser Befund zentral, weil er zeigt, dass standardisierte Interoperabilität allein keine langfristige kryptographische Sicherheit gewährleistet und die Migration zu moderneren Sicherheitsmechanismen erforderlich macht.

    \item \textbf{Gaggero et al. (2024) -- \textit{A Survey on Wireless-Communication Vulnerabilities of ERTMS in the Railway Sector}:}
    Gaggero et al. bieten einen systematischen Survey über drahtlose Kommunikationsschwachstellen im ERTMS-Standard und unterteilen diese in Nahbereichs- (Eurobalise) und Fernfeldverbindungen (GSM-R) \parencite[S. 52--61]{gaggeroSurvey2024}. Sie arbeiten das operative Paradoxon der \textit{Intrinsic Safety} heraus: Da ein Kommunikationsverlust aus Safety-Gründen automatisch einen Zughalt erzwingt, wird das System anfällig für Denial-of-Service- und Jamming-Angriffe \parencite[S. 56--59]{gaggeroSurvey2024}. Ihre Arbeit liefert die grundlegende Taxonomie für das Spannungsfeld zwischen Safety und Security.
\end{itemize}

\subsubsection{Funkbasierte Zugsteuerungsarchitekturen und Systemmodelle}
\begin{itemize}
    \item \textbf{Liu et al. (2020) / Xiang Liu et al. -- \textit{Cyber Security Risk Management for Connected Railroads}:}
    Liu et al. entwickeln im Auftrag der US-amerikanischen Federal Railroad Administration (FRA) eine Risikomanagement-Methodik für vernetzte Zugsicherungssysteme (ATCS, CBTC, PTC) \parencite[S. 1--15]{xiangliuCyber2020}. Sie definieren das generische Komponentenmodell funkbasierter Zugsteuerung (OBU, Balisen, Funkschnittstelle, zentrale Steuerungsrechner) und analysieren Angriffsvektoren wie Eavesdropping und Blue Block Spoofing \parencite[S. 10--15, 46--48]{xiangliuCyber2020}. Das Modell dient in dieser Arbeit als herstellerneutraler Vergleichsrahmen.

    \item \textbf{Iwamoto et al. (2024 / 2025) -- \textit{Train Control System Independent of Communication Transmission Paths}:}
    Iwamoto et al. beschreiben die Forschungen des japanischen \textit{Railway Technical Research Institute} (RTRI) zur Entkopplung der Sicherheitssteuerung von privaten Übertragungsnetzen (LCX-Kabeln) \parencite[S. 1--5]{iwamotoTrain2024}. Auf Basis der IEC 62280 verlagert das Konzept die Integritätsprüfung vollständig auf die Logikschicht mittels HMAC-SHA256 (256 Bit Sicherheitsstärke), um öffentliche Netze (5G, Wi-Fi) sicher nutzen zu können \parencite[S. 10--12]{iwamotoTrain2024}. Diese Arbeit repräsentiert das modernste japanische Paradigma der trägerunabhängigen Zugsteuerung.

    \item \textbf{Tsukisawa et al. (2024) -- \textit{Standardization of Cybersecurity of Operational Technology Systems on Railways}:}
    Tsukisawa et al. analysieren die internationale Standardisierung der OT-Cybersecurity im Bahnsektor basierend auf der IEC 62443 und der zukünftigen IEC/TC 9/PT 63452 \parencite[S. 41--43]{h.tsukisawaStandardization2024}. Sie untersuchen die Segmentierung in Sicherheitszonen (\textit{Zones}) und \textit{Conduits} zur Verhinderung unbefugter Datenflüsse in Leit- und Sicherungsanlagen.
\end{itemize}

\subsubsection{High-Assurance-Architekturen, Angriffserkennung und MTD}
\begin{itemize}
    \item \textbf{Schulz et al. (2019) \& Tverdyshev (2019) -- \textit{Integration Approach for CBTC Applications in High Assurance Security Architecture} / \textit{certMILS}:}
    Schulz et al. und Tverdyshev demonstrieren die Anwendung des MILS-Architekturmusters (\textit{Multiple Independent Levels of Security}) mit zertifizierten Separation Kernels (PikeOS) für Zugsicherungsrechner \parencite[S. 31--35]{schulzIntegration2019}\parencite[S. 80--83]{tverdyshevCertMILS2019}. Die strikte Hardware-Partitionierung schützt die Safety-Logik vor kompromittierten Kommunikationsmodulen und ermöglicht eine kompositionelle Zertifizierung zur Entkopplung unterschiedlicher System-Lebenszyklen \parencite[S. 32--34]{schulzIntegration2019}.

    \item \textbf{Melaragno et al. (2016) -- \textit{Rail Radio Intrusion Detection System (RRIDS) for CBTC}:}
    Melaragno et al. entwickeln ein bahnspezifisches Angriffserkennungssystem (RRIDS), das Funktelegramme passiv auf Protokollebene überwacht und Anomalien wie Replay-, Guessing- oder Korruptionsangriffe mittels $\mu$TESLA echtzeitnah identifiziert, ohne Latenzen in den Steuerungszirkel einzubringen \parencite[S. 39--48]{melaragnoRail2016}.

    \item \textbf{Poschinger et al. (2020) -- \textit{OpenMTD: A Framework for Efficient Network-Level MTD Evaluation}:}
    Poschinger et al. evaluieren Moving Target Defense (MTD) wie Port Hopping und IP Address Shuffling auf Netzwerkebene \parencite[S. 66--68, 121]{poschingerOpenMTD2020}. Sie zeigen die Grenzen von MTD im Bahn-Echtzeitbetrieb auf, da dynamische Adressänderungen kritische Latenz- und Synchronisationsrisiken erzeugen können \parencite[S. 121]{poschingerOpenMTD2020}.
\end{itemize}

\subsubsection{Umfassende Systemliteratur}
\begin{itemize}
    \item \textbf{Trinckauf et al. (2024) -- \textit{ETCS in Deutschland} (2. Auflage):}
    Das Standardwerk von Trinckauf et al. stellt die fachliche Gesamtgrundlage für das europäische Zugsicherungssystem ETCS, die RMR/FRMCS-Kommunikation, Baseline 4, TLS 1.3 sowie die betrieblichen Sicherheitsanforderungen nach RAMS dar \parencite[S. 49, 123, 204--211]{trinckaufETCS2024}.
\end{itemize}
...
Die von Chothia et al. nachgewiesenen Schwachstellen des traditionellen \textit{EuroRadio}-Integritätsschutzes verdeutlichen ein grundsätzliches Problem langlebiger Eisenbahnarchitekturen: Sicherheitskritische Steuerungssoftware besitzt wesentlich längere Lebenszyklen als die für ihre Kommunikation eingesetzten kryptographischen Verfahren~\parencite[S. 751--754]{chothiaAttack2017}. Damit entsteht ein strukturelles Spannungsfeld zwischen langfristiger \textit{Safety}-Zertifizierung und der Notwendigkeit, \textit{Security}-Mechanismen regelmäßig an neue Bedrohungen und kryptographische Standards anzupassen.

Eine Möglichkeit, diesen unterschiedlichen Lebenszyklen architektonisch zu begegnen, ist die Isolation von Kommunikations- und Sicherheitsdiensten von der eigentlichen \textit{Safety}-Logik. Hier setzt das \gls{mils}-Architekturmuster (\textit{Multiple Independent Levels of Security}) an (siehe \autoref{sec:Sichere_Software-Partitionierung_MILS_und_Separation_Kernels})~\parencite[S. 31--32]{schulzIntegration2019}:

\begin{enumerate}
    \item \textbf{Schutz vor Privilegienerweiterung und Durchgreifen:} Ein zertifizierter \textit{Separation Kernel} unterteilt den Fahrzeugrechner (\gls{evc}) in strikt isolierte \textit{Hardware}-Partitionen. Selbst wenn ein Angreifer Schwachstellen im \gls{gsm-r}-Modem, im Protokollstapel oder bei der Schlüsselverwaltung (\gls{kmc}) ausnutzt, begrenzt die \gls{mils}-Isolierung die Auswirkungen auf die Kommunikationspartition und erschwert ein Durchgreifen auf die sicherheitskritische Steuerungsebene (\textit{Safety-Logic})~\parencite[S. 32--34]{schulzIntegration2019}~\parencite[S. 80--81]{tverdyshevCertMILS2019}.
    \item \textbf{Entkopplung der Lebenszyklen:} \gls{mils} ermöglicht eine kompositionelle Zertifizierung. Kryptographische Protokolle oder Funktreiber (mit Lebenszyklen von 2--5 Jahren) können aktualisiert und gepatcht werden, ohne dass der \textit{Safety}-Sicherheitsnachweis der Kernsteuerungssoftware (mit Lebenszyklen von über 20 Jahren) vollständig neu geführt werden muss~\parencite[S. 31, 35]{schulzIntegration2019}~\parencite[S. 83]{tverdyshevCertMILS2019}.
\end{enumerate}

Damit verschiebt sich die Bewertung von der Frage, ob ein einzelnes Kommunikationsprotokoll dauerhaft sicher bleibt, hin zur Frage, wie gut die Gesamtarchitektur den Austausch und die Aktualisierung sicherheitsrelevanter Kommunikationskomponenten zulässt. Dieser Gedanke verbindet die \gls{mils}-basierte Kapselung mit der in \gls{frmcs} angestrebten funktionalen Entkopplung von Anwendung und Übertragungsmedium.
...
Die Überwindung der \gls{gsm-r}- und \textit{EuroRadio}-bedingten Altlasten vollzieht sich in Europa durch die Migration zu \gls{frmcs} im Rahmen von \gls{rmr}~\parencite[S. 49, 123]{trinckaufETCS2024}. Anstelle einer weiteren Verlängerung bestehender kryptographischer Mechanismen wird die Kommunikation auf moderne, \gls{ip}-basierte Sicherheitsdienste mit \gls{tls} und \gls{pki}-gestützter Authentisierung ausgerichtet~\parencite[S. 105, 211]{trinckaufETCS2024}. Entscheidend ist dabei weniger ein einzelner Algorithmus als die architektonische Fähigkeit, Sicherheitsmechanismen unabhängig von der eigentlichen \gls{etcs}-Anwendungslogik weiterzuentwickeln.
