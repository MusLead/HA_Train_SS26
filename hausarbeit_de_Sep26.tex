...
%TODO sorting lagi source yang memang tidak perlu dipakai... terlalu banyak menurut aku
Sicherheitskritische Eisenbahnkommunikation bezeichnet die Übertragung von Informationen, die Zugbewegungen oder die betriebliche Sicherheit beeinflussen können 
% \parencite[S. 118]{trinckaufETCS2024}
% \parencite[S. 743]{chothiaAttack2017}. 
Beispiele sind \textit{Movement Authorities}, Bremsinformationen, Streckenstatus, Geschwindigkeitsprofile, Zugpositionsmeldungen, Notstopp-Nachrichten und betriebliche Anweisungen 
\unskip~\parencite[S. 120--121]{trinckaufETCS2024}
% \parencite[S. 14]{iwamotoTrain2024}
% \parencite[S. 55, 57]{gaggeroSurvey2024}. 
Wenn solche Informationen verändert, verzögert, wiederholt oder gefälscht werden, kann das System falsche Entscheidungen treffen oder in einen restriktiven Zustand wechseln 
\unskip~\parencite[S. 17]{iwamotoTrain2024}
% \parencite[S. 56, 58]{gaggeroSurvey2024}
% \parencite[S. 204]{trinckaufETCS2024}
% \parencite[S. 746]{chothiaAttack2017}. 
Eisenbahnkommunikation muss daher nicht nur als technischer Datenaustausch, sondern als Teil eines größeren sicherheitskritischen Regelkreises analysiert werden. 
% \parencite[S. 118, 204]{trinckaufETCS2024}.
...
Ein wesentlicher Unterschied zwischen \textit{Safety} und \textit{Security} liegt in der zeitlichen Dynamik: Während \textit{Safety}-Anforderungen aufgrund physikalischer und statistischer Konstanten über lange Zeiträume statisch bleiben, erfordert die \textit{Security} aufgrund der technologischen Evolution eine kontinuierliche Anpassung der Schutzmechanismen~\parencite[S. 204]{trinckaufETCS2024}. 
% In der Systemarchitektur übernimmt die Security die Rolle eines Dienstleisters für die Safety, um die RAMS-Eigenschaften (Reliability, Availability, Maintainability, Safety) über den gesamten Lebenszyklus aufrechtzuerhalten \parencite[S. 204]{trinckaufETCS2024}. 
Technisch manifestiert sich dieses Zusammenspiel im Prinzip der \textit{Intrinsic Safety} (inhärente Sicherheit): Da das System bei einem Kommunikationsabriss keine gesicherten Informationen über den Fahrweg erhalten kann, löst die Sicherheitslogik automatisch einen restriktiven Zustand – in der Regel den Zughalt – aus, um Gefährdungen wie Kollisionen zu vermeiden~\parencite[S. 56]{gaggeroSurvey2024}. Dieser fahrwegsicherungstechnische Kausalzusammenhang führt dazu, dass vorsätzliche Eingriffe in die Kommunikationsverfügbarkeit, etwa durch Funkstörungen (\textit{Jamming}), unmittelbar die Sicherheitsreaktion des Systems triggern. In der Folge wird die funktionale Sicherheitslogik zum Mechanismus eines \textit{Denial-of-Service}-Zustands, der die operative Verfügbarkeit des Gesamtsystems einschränkt~\parencite[S. 207]{trinckaufETCS2024}~\parencite[S. 56, 59]{gaggeroSurvey2024}.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{res/Bahn_Infrastruktur.jpg}
\caption{Überblick über die Schieneninfrastruktur~\cite{xiangliuCyber2020}}
\label{fig:Schieneninfrastruktur}
\end{figure}

\begin{itemize}
    \item \textbf{Fahrzeugseitige Steuerungseinheit (\textit{On-Board Unit} / \gls{evc} / \gls{atc}-Fahrzeuggerät):} Ein zentraler fahrzeugseitiger Sicherheitsrechner, der die Ist-Geschwindigkeit sowie die mittels Odometrie, Radsensoren, Doppler-Radar oder Satellitenortung ermittelte Zugposition verarbeitet~\parencite[S. 10]{xiangliuCyber2020}~\parencite[S. 101, 104]{trinckaufETCS2024}. Neben der kontinuierlichen Berechnung und Überwachung dynamischer Bremskurven zur Durchsetzung von Fahrterlaubnissen dient die \gls{obu} über das Führerraumdisplay (\textit{Driver Machine Interface}, \gls{dmi}) der optischen und akustischen Zustandüberwachung für das Fahrpersonal~\parencite[S. 10]{xiangliuCyber2020}~\parencite[S. 102--103]{trinckaufETCS2024}. Je nach Ausprägung (z. B. \gls{etcs} \textit{Level} 1--3, \gls{d-atc} oder \gls{atacs}) übernimmt sie dabei sowohl überwachende als auch eingreifende Steuerungsfunktionen zur Sicherung des Zuglaufs~\parencite[S. 14]{xiangliuCyber2020}~\parencite[S. 62--66]{trinckaufETCS2024}.
    \item \textbf{Gleisseitige punktförmige Datenübertrager (Balisen / \textit{Transponder}):} Im Gleisbett montierte passive oder aktive \textit{Transponder}, die beim Überfahren per Induktion Telegramme mit dem Fahrzeug austauschen~\parencite[S. 10--11]{xiangliuCyber2020}~\parencite[S. 75--76]{trinckaufETCS2024}. Sie dienen der fahrzeugseitigen \gls{obu} nicht nur als absolute Referenzpunkte zur Rekalibrierung von Kumulationsfehlern der Wegmessung, sondern übertragen auch operative Steuerungsinformationen~\parencite[S. 75, 79]{trinckaufETCS2024}. Je nach Systemebene übermitteln sie variabel generierte Fahrterlaubnisse (z. B. in \gls{etcs} \textit{Level} 1 angesteuert über \textit{Lineside Electronic Units}), Geschwindigkeits- und Neigungsprofile oder lösen bei Halt zeigenden Signalen bzw. unzulässigen Fahrten eine automatische Zwangsbremsung (\textit{Trip}) aus~\parencite[S. 62--63, 75, 253]{trinckaufETCS2024}~\parencite[S. 55]{gaggeroSurvey2024}.
    \item \textbf{Drahtlose Kommunikationsschnittstelle:} Eine bidirektionale Funkschnittstelle zwischen Zug und Strecke zur kontinuierlichen Übertragung von Positionsberichten (vom Zug an die Strecke) und Fahrterlaubnissen (von der Strecke an den Zug)~\parencite[S. 9--10]{xiangliuCyber2020}. 
    In den meisten weltweiten Eisenbahnnetzen basiert die drahtlose Kommunikationsschnittstelle derzeit noch auf Mobilfunkstandards der 2. Generation (\gls{2g})~\parencite[S. 12, 14]{xiangliuCyber2020}. Im europäischen Raum wird hierzu \gls{gsm-r} (\textit{Global System for Mobile Communications -- Railway}) genutzt, während in anderen Regionen wie Japan schmalbandige \gls{tdma}-Verfahren (\textit{Time Division Multiple Access}) oder \textit{Leaky-Coaxial}-Kabel (\gls{lcx})~\parencite[S. 12--14]{xiangliuCyber2020}. 
    \item \textbf{Optionale optische Signalisierung:} Ortsfeste Lichtsignale an der Strecke, die in bestimmten Ausbaustufen beibehalten werden, um dem Triebfahrzeugführer eine visuelle Gegenprüfung der fahrzeugseitig angezeigten Fahrterlaubnis zu ermöglichen~\parencite[S. 10]{xiangliuCyber2020}.
\end{itemize}
...
Die Übertragung von Steuerungsdaten zwischen Zug und \gls{rbc} erfolgt über einen mehrschichtigen Protokollstapel~\parencite[S. 744]{chothiaAttack2017}:

\begin{enumerate}
    \item \textbf{Untere Schicht (\gls{gsm-r} / \gls{rmr}):} Bietet die physikalische und netzwerkbezogene Übertragungsinfrastruktur. Verschlüsselungsverfahren auf Funkebene (wie \gls{a51} oder \gls{a53}) sichern die Verbindung zwischen Fahrzeugantenne und Basisstation (\gls{bts})~\parencite[S. 744, 750]{chothiaAttack2017}.
    \item \textbf{Mittlere Schicht (\textit{EuroRadio}):} Diese Schicht ist für die Authentifizierung und Integritätssicherung der \gls{etcs}-Anwendungsdaten zuständig. Sie nutzt kryptographische \textit{Message Authentication Codes} (\gls{mac}s), um sicherzustellen, dass die Daten von einem autorisierten Absender stammen~\parencite[S. 744, 751]{chothiaAttack2017}.
    \item \textbf{Höchste Schicht (\gls{etcs}-Anwendungsschicht):} Verarbeitet die betrieblichen Datensätze (z. B. Paket 12 für \textit{Movement Authorities}, Paket 0/1 für Positionsberichte). Sie ist als zustandsbehaftetes Protokoll konzipiert und nutzt relative Zeitstempel sowie Sequenznummern zur Erkennung veralteter oder duplizierter Telegramme~\parencite[S. 744, 754]{chothiaAttack2017}.
\end{enumerate}
