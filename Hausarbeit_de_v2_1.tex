...
\begin{figure}[htbp!]
\centering
\includegraphics[width=0.80\textwidth]{res/Bahn_Infrastruktur.jpg}
...
\subsection{Kriterienbasierte Gegenüberstellung}
\label{sec:Kriterienbasierte_Gegenüberstellung}
Die Anwendung der in Abschnitt~\ref{sec:Bewertungsverfahren} definierten Kriterien ergibt kein einfaches „sicherer“ oder „unsicherer“. Vielmehr zeigen sich unterschiedliche Stärken, Abhängigkeiten und Reifegrade. Tabelle~\ref{tab:vergleich-europa-japan} fasst die für die weitere Diskussion maßgeblichen Befunde zusammen.

\begin{table}[htbp]
\centering
\caption{Kriterienbasierte Gegenüberstellung der untersuchten europäischen und japanischen Ansätze}
\label{tab:vergleich-europa-japan}
\begin{tabularx}{\textwidth}{@{} p{0.22\textwidth} X X @{} }
\toprule
\textbf{Kriterium} & \textbf{Europa} & \textbf{Japan} \\
\midrule
Standardisierung und Interoperabilität & Starker gemeinsamer Rahmen durch \gls{ertms}/\gls{etcs}; erleichtert grenzüberschreitende Kompatibilität, schafft aber gemeinsame Legacy-Abhängigkeiten. & Betreiber- und streckenspezifischer; geringere Vereinheitlichung, dadurch weniger direkte Übertragbarkeit von Implementierungen und Updates. \\
Integrität und Authentizität & Traditionell \textit{EuroRadio}-\gls{mac}; Migration zu moderneren \gls{tls}/\gls{pki}-basierten Mechanismen. & Im untersuchten \gls{rtri}-Ansatz \gls{hmac-sha256}; operative Betreiberdetails bleiben teilweise nicht öffentlich. \\
Verfügbarkeit und Resilienz & Hohe Abhängigkeit kontinuierlicher Funkkommunikation in Level~2/3; \textit{Fail-Safe} begrenzt Safety-Risiken, kann aber Betriebsunterbrechungen verstärken. & Teilweise physisch stark kontrollierte Übertragungswege; Forschungsansatz erlaubt verschiedene Träger, muss deren Ausfälle jedoch logisch beherrschen. \\
Entkopplung und Aktualisierbarkeit & \gls{frmcs}, standardisierte Gateways und High-Assurance-Partitionierung zielen auf austauschbare Kommunikations- und Security-Komponenten. & \gls{rtri}-Ansatz trennt Sicherheitslogik explizit vom Übertragungsweg; Umsetzung und Skalierung sind noch weniger breit belegt. \\
Evidenz und Reifegrad & Umfangreiche Standards, produktive Bestandsnetze und dokumentierte Migration. & Mischung aus produktiven betreiberspezifischen Systemen und veröffentlichten Forschungsansätzen; geringere öffentliche Detailtiefe. \\
\bottomrule
\end{tabularx}
\end{table}

Aus dieser Gegenüberstellung folgt, dass der wichtigste Unterschied nicht in einem einzelnen kryptographischen Algorithmus liegt. Entscheidend ist vielmehr, wie weit eine Architektur Kommunikationsdienste, Sicherheitsmechanismen und \textit{Safety}-Logik voneinander entkoppelt und wie kontrolliert diese Komponenten über lange Betriebszeiträume aktualisiert werden können.
...
Der Vergleich beantwortet die Forschungsfrage in drei Punkten. Erstens schützen beide Regionen sicherheitskritische Kommunikation durch eine Kombination aus protokollseitiger Integritäts- und Authentizitätssicherung, \textit{Fail-Safe}-Verhalten und architektonischer Trennung. Zweitens unterscheiden sich die Ausgangsbedingungen: Europa priorisiert standardisierte Interoperabilität und muss deshalb Legacy-Kompatibilität kontrolliert abbauen; Japan verfügt über größere betreiberspezifische Autonomie, muss dafür aber heterogene Sicherheitspraktiken und begrenzte Vergleichbarkeit beherrschen. Drittens verschiebt sich die zentrale Sicherheitsfrage zunehmend von „Welcher Funkstandard ist sicher?“ zu „Wie kann die Safety-Logik unabhängig von wechselnden und potenziell unzuverlässigen Kommunikationsmedien sicher bleiben?“
